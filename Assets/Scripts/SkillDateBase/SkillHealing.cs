...
public class SkillHealing : MonoBehaviour {
    ...
    void Start()
    {
        ...
        reader = sql.ReadOneTable("SkillDatabase", new string[] { "SkillName" }, new string[] { "==" }, new string[] { "'Healing'" });
        while (reader.Read())
        {
    ...
    void Update()
    {
        if (SkillChoose && SkillCoolDown == 0)
        {
            ...
            Destroy(Magic, 3f); Magic.transform.position = new Vector3(TriggerPlayer.transform.position.x, TriggerPlayer.transform.position.y + 7f, TriggerPlayer.transform.position.z);
