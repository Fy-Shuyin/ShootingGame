...
public class SkillTheLightOfSword : MonoBehaviour {
    ...
    void Start ()
    {
        ...
        reader = sql.ReadOneTable("SkillDatabase", new string[] { "SkillName" },new string[] {"=="},new string[] { "'TheLightOfSword'" });
        while (reader.Read())
        {
    ...
    void Update()
    {
        if (SkillChoose && SkillCoolDown == 0)
        {
            ...
            Destroy(Magic, 5f);
            Magic.transform.position = new Vector3(TriggerPlayer.transform.position.x, TriggerPlayer.transform.position.y + 4f, TriggerPlayer.transform.position.z);
            Magic.transform.position = Magic.transform.position + TriggerPlayer.transform.forward * 5f;
